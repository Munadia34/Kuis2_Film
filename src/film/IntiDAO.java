...
public class IntiDAO {
    ...
    public void Create(IntiModel Model){
        try{
            String query = "INSERT INTO data_inti VALUES ('','"+Model.getJudul()+"','"+Model.getTipe()+"','"+Model.getEpisode()+"','"+Model.getGenre()+"','"+Model.getStatus()+"','"+Model.getRating()+"')";
    ...
    public void Delete(IntiModel Model){
        try{
    ...
    public String[][] readData(){
            try{
                ...
                while(resultSet.next()){
                    int kode = resultSet.getInt(1);
                    data[jmlData][0] = Integer.toString(kode+1);
    ...
    public String[][] searchData(IntiModel model){
            try{
                int jmlData = 0, wew;
                String data[][] = new String[getJmldata()][3];
                String query = "SELECT * FROM data_inti WHERE judul = '"+model.getJudul()+"'";
                ResultSet resultSet = statement.executeQuery(query);
                while(resultSet.next()){
                    wew = jmlData+1;
